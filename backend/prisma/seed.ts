...
async function main() {
  ...
  const chemicalsType = await prisma.inventoryType.create({
    data: {
      ...
      singularAr: 'مادة كيميائية',
      descriptionAr: 'مادة أولية للإنتاج. تُتبع بالدُفعات (FEFO بسبب انتهاء الصلاحية).',
      hasBatches: true,
      hasExpiry: true,
  ...
  const tigeType = await prisma.inventoryType.create({
    data: {
      ...
      // "tige" is the upper of a shoe — الوجه العلوي, not a transliteration
      // (see apps/dashboard/src/lib/i18n/stock.ts's translator note).
      labelAr: 'الوجه العلوي للأحذية',
      singularAr: 'الوجه العلوي',
      descriptionAr: 'مادة أولية للإنتاج. بدون تاريخ صلاحية ولا دُفعات. المتغيرات: اللون + المقاس.',
      hasBatches: false,
      hasExpiry: false,
      isProductionInput: true,
      hasColor: true,
  ...
  const sparePartsType = await prisma.inventoryType.create({
    data: {
      ...
      labelAr: 'قطع الغيار',
      singularAr: 'قطعة غيار',
      descriptionAr: 'مخزون صيانة الآلات. لا يدخل أبدًا في الإنتاج. يُتابع فيه التخزين والآلة ومستوى الأهمية.',
      hasBatches: false,
      hasExpiry: false,
  ...
  const finishedGoodsType = await prisma.inventoryType.create({
    data: {
      ...
      singularAr: 'منتج نهائي',
      descriptionAr: 'ما تنتجه المصنع — الدرجة الأولى، الدرجة الثانية والتالف. المتغيرات: اللون، المقاس، الجنس. سعر البيع بالدينار الجزائري. يُغذَّى من وحدة الإنتاج.',
      hasBatches: false,
      hasExpiry: false,
      isProductionInput: false,
      hasColor: true,
